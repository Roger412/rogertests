...
void PrintErrorMessage(unsigned int errorCode);
...
int main(int argc, char** argv)
{
	...
	try
	{
		...
		// std::ofstream file(DATA_FILE, std::ios_base::binary); <-- binary
		...
		try
		{
			...
			for (uint32_t i = 0; i < numberOfChannelsToAcquire; ++i) {
				file << configuration.Channels[i].name;
				if (i < numberOfChannelsToAcquire - 1)
					file << ",";
			}
			file << "\n";
			...
			for (int i = 0; i < numberOfGetDataCalls; i++)
			{
				...
				// Write data to file.
				// file.write((const char*) acquisitionBuffer, acquisitionBufferLength * sizeof(float)); <-- binary
					// Write one row of comma-separated floats
				for (int ch = 0; ch < acquisitionBufferLength; ++ch) {
					file << acquisitionBuffer[ch];
					if (ch < acquisitionBufferLength - 1)
						file << ",";
				}
				file << "\n";
