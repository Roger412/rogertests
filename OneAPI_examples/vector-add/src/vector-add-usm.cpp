...
void VectorAdd(queue &q, const int *a, const int *b, int *sum, size_t size) {
  ...
  // Launch kernel and capture the event
  sycl::event e = q.parallel_for(num_items, [=](auto i) {
    sum[i] = a[i] + b[i];
  });
  ...
  // Extract profiling info in nanoseconds
  ...
  double duration_ms = static_cast<double>(end - start) / 1e6;

  std::cout << "⏱ GPU kernel execution time: " << duration_ms << " ms\n";
}
